                            ...
                            if let (Some(start_name_node), Some(end_name_node)) = (
                                ...
                                if start_name == "include" {
                                    errors.push(ValidationError {
                                        ...
                                        message:
                                            "Invalid include: tag 'include' must be self-closing"
                                                .to_string(),
                                    });
                                }
                                if start_name == "meta" {
                ...
                if child.kind() == "attribute" {
                    if let (Some(name_node), Some(val_node)) = (child.child(0), child.child(2)) {
                        ...
                        let attr_val = contents[val_node.byte_range()]
                            ...
                            .trim_matches('\'')
                            .to_string();
                        if (attr_name == "id" || attr_name == "alias" || attr_name == "class")
                            && attr_val == anchor
                        {
                            return true;
                        }
                                ...
                                (child.child(0), child.child(2))
                            {
                                ...
                                if attr_name == "href" {
                                    ...
                                    let target_file = if let Some(path) = target_file_path {
                                        if let Some(parent) = parent_path {
                                            ...
                                            workspace
                                                .files(db)
                                                .into_iter()
                                                .find(|f| f.path(db) == full_path_str)
                                        } else {
                                            None
                                        }
                                    } else {
                                    ...
                                    match target_file {
                                        None => {
                                            errors.push(ValidationError {
                                                range: crate::parser::ts_range_to_lsp(
                                                    val_node.range(),
                                                ),
                                                message: format!(
...
fn validate_value_type(
    ...
    match type_name {
        ...
        "Color" => match value {
            HubValue::Number(num_str) => num_str.starts_with("0x") || num_str.starts_with("0X"),
            HubValue::String(s) => {
