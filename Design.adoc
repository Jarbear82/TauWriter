...
* *Is-A*:
  ** `gpui_kit::Render` implemented on view components.
...
│   │   ├── raw_editor.rs (gpui_kit::component::input::Input code mode)
...
* *Implementation*:
  ...
  3. Render the left half containing `DocumentView` and the right half containing `GraphPaneView` using `gpui_kit::component::resizable::h_resizable("workspace-split")`.
...
* *Implementation & Phased Block Editor Pipeline*:
  1. Raw Mode (`RawEditor`): Render the raw XML text editor with syntax highlighting using `gpui_kit::component::input::Input`. Render the `<include src="..." />` tags literally.
