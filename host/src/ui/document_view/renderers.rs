...
use gpui::{div, prelude::*, rgb, px, AnyElement, Context, Entity};
        ...
        Block::Paragraph { runs, id, attributes, range: _ } => {
            ...
            div()
                ...
                .children(runs.iter().enumerate().map(|(run_idx, run)| {
                    render_run(document_home, input_state, run, idx, run_idx, &doc_blocks, cx)
                }))
                .into_any_element()
        }
                    ...
                    .child(
                        div()
                            ...
                            .children(item.runs.iter().enumerate().map(|(run_idx, run)| {
                                render_run(document_home, input_state, run, idx, run_idx + 100 * item_idx, &doc_blocks, cx)
                            }))
                    )
            });
                    ...
                    table_row = table_row.child(
                        div()
                            ...
                            .children(cell.iter().enumerate().map(|(run_idx, run)| {
                                render_run(document_home, input_state, run, idx, run_idx + 100 * cell_idx + 1000 * row_idx, &doc_blocks, cx)
                            }))
                    );
                }
...
pub(crate) fn element_tooltip(tag_name: &str, id: &Option<String>, attributes: &[(String, String)]) -> String {
