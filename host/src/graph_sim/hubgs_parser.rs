...
fn parse_definition_line(
    ...
    if trimmed.contains("ALLOWS [") {
        let (arrow_end, rel_name, arrow_str) = if let Some(double_arrow_idx) = trimmed.find("<->") {
            (
                double_arrow_idx + 3,
                trimmed[..double_arrow_idx].trim().to_string(),
                "<->".to_string(),
            )
        } else if let Some(arrow_idx) = trimmed.find("->") {
            (
                arrow_idx + 2,
                trimmed[..arrow_idx].trim().to_string(),
                "->".to_string(),
            )
        } else if let Some(rarrow_idx) = trimmed.find("<-") {
            (
                rarrow_idx + 2,
                trimmed[..rarrow_idx].trim().to_string(),
                "<-".to_string(),
            )
        } else if let Some(dash_idx) = trimmed.find(" - ") {
            (
                dash_idx + 3,
                trimmed[..dash_idx].trim().to_string(),
                "-".to_string(),
            )
        } else if let Some(dash_idx) = trimmed.find('-') {
        ...
        if let Some(allows_idx) = trimmed.find("ALLOWS [") {
            ...
            if min.is_empty() || max.is_empty() {
                anyhow::bail!(
                    "Invalid HubGS syntax: multiplicity bounds cannot be empty: {}",
                    trimmed
                );
            }
            ...
            if let Some(end_bracket) = target_part.find(']') {
                ...
                anyhow::bail!(
                    "Invalid HubGS syntax: missing closing bracket in target: {}",
                    trimmed
                );
            }
        }
