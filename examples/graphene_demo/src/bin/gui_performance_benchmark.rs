...
use gpui_kit::{px, Application, Bounds, IntoElement, Render, Styled, WindowBounds, WindowOptions};
...
use graphene_layout::{
    force_atlas2_step, BipartiteLayout, CircleLayout, CircularAdvancedLayout,
    CollisionForceDirectedLayout, ConcentricHubLayout, CoseLayout, FA2Edge, FA2Node, FA2Settings,
    ForceDirectedLayout, GridSortedLayout, KamadaKawaiLayout, Layout, MdsLayout,
    ReingoldTilfordLayout, SugiyamaLayout, TutteBarycentricLayout,
};
...
impl GuiPerformanceBenchmarkApp {
    pub fn new(window: &mut gpui_kit::Window, _cx: &mut gpui_kit::Context<Self>) -> Self {
        let screen_bounds: Bounds<f32> = Bounds {
            origin: gpui_kit::point(0.0, 0.0),
            size: gpui_kit::size(1280.0, 800.0),
        };
    ...
    fn rebuild_and_run_layout(&mut self) {
        ...
        for i in 0..self.scale {
            ...
            let hub2 = i / 10;
            self.state
                .add_edge(nodes[i], nodes[hub1], EdgeData::default());
            if hub2 != hub1 && hub2 < self.scale {
                self.state
                    .add_edge(nodes[i], nodes[hub2], EdgeData::default());
            }
            if i + 1 < self.scale {
                self.state
        ...
        match self.selected_algo.as_str() {
            ...
            "BipartiteLayout" => {
                let n_map: HashMap<NodeId, usize> = self
                    .state
                    .node_index_to_id
                    .iter()
                    .enumerate()
                ...
                let mut layout = BipartiteLayout {
                    partition_fn: move |id| {
                        if *n_map.get(&id).unwrap_or(&0) % 2 == 0 {
                            0
                        } else {
                            1
                        }
            ...
            "ForceAtlas2" => {
                ...
                let mut fa2_nodes: Vec<FA2Node> = (0..n_nodes)
                    .map(|i| {
                        let p = *self.state.positions.get(i);
                ...
                for i in 0..n_nodes {
                    self.state.positions.set(
                        i,
                        Vec2::new(fa2_nodes[i].pos.x as f32, fa2_nodes[i].pos.y as f32),
                    );
                }
            }
            "CoseLayout" => {
...
impl Render for GuiPerformanceBenchmarkApp {
    fn render(
        ...
        gpui_kit::div()
            .size_full()
            .bg(gpui_kit::rgba(0x0f172aff)) // Dark navy slate background
            .flex()
            .flex_col()
            .child(
                ...
                gpui_kit::div()
                    .w_full()
                    .bg(gpui_kit::rgba(0x1e293bff))
                    .p_4()
                    .border_b_1()
                    .border_color(gpui_kit::rgba(0x334155ff))
                    .flex()
                    .flex_col()
                    .gap_3()
                    .child(
                        gpui_kit::div()
                            ...
                            .child(
                                gpui_kit::div()
                                    .text_color(gpui_kit::rgba(0xf8fafcff))
                                    .text_xl()
                                    .font_bold()
                    ...
                    .child(
                        ...
                        gpui_kit::div()
                            ...
                            .child(
                                gpui_kit::div()
                                    .bg(gpui_kit::rgba(0x0f172aff))
                                    .p_3()
                                    .rounded_md()
                                    .child(format!(
                                        "⏱️ Layout Compute: {:.3} ms",
                                        self.layout_compute_ms
                                    ))
                                    .text_color(gpui_kit::rgba(0x38bdf8ff)),
                            )
                            .child(
                                gpui_kit::div()
                                    .bg(gpui_kit::rgba(0x0f172aff))
                                    ...
                                    .child(format!(
                                        "🎨 Element Build: {:.3} ms",
                                        self.render_build_ms
                                    ))
                                    .text_color(gpui_kit::rgba(0x4ade80ff)),
                            )
                            .child(
                                gpui_kit::div()
                                    .bg(gpui_kit::rgba(0x0f172aff))
                                    ...
                                    .child(format!(
                                        "👁️ Visible / Total: {} / {}",
                                        self.visible_node_count, self.scale
                                    ))
                                    .text_color(gpui_kit::rgba(0xfacc15ff)),
                            ),
                    ),
            )
            .child(
        ...
        cx.open_window(
            WindowOptions {
                window_bounds: Some(WindowBounds::Windowed(gpui_kit::Bounds::centered(
                    None,
                    gpui_kit::size(gpui_kit::px(1280.0), gpui_kit::px(800.0)),
                    cx,
                ))),
